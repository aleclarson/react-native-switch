...
import { a, SpringValue } from 'react-haru/native';
import {
  View,
  Text,
  StyleSheet,
...
export class Switch extends Component {
  ...
  static defaultProps = {
    ...
    circleActiveBorderColor: 'rgb(100, 100, 100)',
    circleInactiveBorderColor: 'rgb(80, 80, 80)',
    circleSize: 30,
    barHeight: null,
    circleBorderWidth: 1,
  ...
  constructor(props, context) {
    ...
    this.state = {
      value: props.value,
      transformSwitch: new SpringValue(
        props.value
          ? props.circleSize / props.switchLeftPx
          : -props.circleSize / props.switchRightPx
      ),
      backgroundColor: new SpringValue(props.value ? 75 : -75),
      circleColor: new SpringValue(props.value ? 75 : -75),
      circleBorderColor: new SpringValue(props.value ? 75 : -75),
    };
  }
  ...
  animateSwitch = (value, cb = () => {}) => {
    Promise.all([
      this.state.transformSwitch.start(
        value
          ? this.props.circleSize / this.props.switchLeftPx
          : -this.props.circleSize / this.props.switchRightPx
      ),
      this.state.backgroundColor.start(value ? 75 : -75),
      this.state.circleColor.start(value ? 75 : -75),
      this.state.circleBorderColor.start(value ? 75 : -75),
    ]).then(cb);
  };
  ...
  render() {
    ...
    const interpolatedColorAnimation = backgroundColor.to(
      [-75, 75],
      [backgroundInactive, backgroundActive]
    );

    const interpolatedCircleColor = circleColor.to(
      [-75, 75],
      [circleInActiveColor, circleActiveColor]
    );

    const interpolatedCircleBorderColor = circleBorderColor.to(
      [-75, 75],
      [circleInactiveBorderColor, circleActiveBorderColor]
    );
